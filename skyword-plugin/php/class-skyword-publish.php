...
// Allow direct db queries for this file.  They have worked in practice with no issue for years and in some cases might be impossible to easily recode.
...
class Skyword_Publish {
		...
		if ( 'success' === $login['status'] ) {
			...
			if ( $taxonomies ) {
				foreach ( $taxonomies as $taxonomy ) {
					...
					$terms          = get_terms( array('taxonomy' => $struct['name'], 'hide_empty' => false));
	...
	public function skyword_delete_post( $args = '' ) {
		...
		if ( 'success' === $login['status'] ) {
			...
			if ( empty( $post['ID'] ) ) {
				return new IXR_Error( 404, __( 'Invalid post ID.', 'skyword-plugin' ) );
			}

			$result = wp_delete_post( $post_id );

			if ( ! $result ) {
				return new IXR_Error( 500, __( 'The post cannot be deleted.', 'skyword-plugin' ) );
			}
	...
	public function skyword_post( $args ) {
		...
		if ( 'success' === $login['status'] ) {
            ...
            if ( null !== $data['wp-user-id'] && is_numeric( trim( $data['wp-user-id'] ) ) ) {
                $new_post['post_author'] = (int) $data['wp-user-id'];
			}
		...
		if ( 'success' === $login['status'] ) {
            ...
            if(array_key_exists('skywordContentId', $data)) {
                ...
                if ( ! empty( $upload['error'] ) ) {
                    $errorString = __( 'Could not write file ', 'skyword-plugin') . $name . '(' . $upload['error']. ')';
    ...
    */
	...
	private function login( $args ) {
		...
		if ( 'skywordapikey' !== $username ) {
			if ( ! $user = $wp_xmlrpc_server->login( $username, $password ) ) {
				$response['message'] = new IXR_Error( 403, __( 'Invalid UN/PW Combination: UN = ', 'skyword-plugin') . $username . __(' PW = ', 'skyword-plugin') . $password );
				$response['status']  = 'error';
			} else if ( ! user_can( $user->ID, 'edit_posts' ) ) {
				$response['message'] = new IXR_Error( 403, __( 'You do not have sufficient privileges to login.', 'skyword-plugin' ) );
				$response['status']  = 'error';
			} else {
        ...
        if ( $temp_time - $timestamp <= 20000 && $temp_time - $timestamp >= - 20000 ) {
            if ( '' !== $api_key ) {
                ...
                if ( $temp_hash === $hash ) {
                    ...
                    $response['message'] = new IXR_Error( 403, __( 'Could not match hash.' , 'skyword-plugin') );
                    $response['status']  = 'error';
                }
            } else {
                $response['message'] = new IXR_Error( 403, __( 'Skyword API key not set.' , 'skyword-plugin') );
                $response['status']  = 'error';
            }
        } else {
            $response['message'] = new IXR_Error( 403, __( 'Bad timestamp used. ', 'skyword-plugin') . $hash . __(' Timestamp sent: ', 'skyword-plugin') . $timestamp );
            $response['status']  = 'error';
        }
	...
	private function generate_author_slug( $data ) {
        return sanitize_text_field(mb_strtolower(str_replace(' ', '-', $data['display-name'])));
    }
		...
		if ( ! $user_id ) {
			...
			if ( ! $user_id ) {
				if ( null !== $coauthors_plus ) {
					...
					if ($options['skyword_coauthors_friendly_pages']) {
                    ...
                    return new IXR_Error( 500, __( 'User cannot be created, ensure new user creation is enabled in both Skyword and WordPress or use the Co-Authors Plus plugin.', 'skyword-plugin' ) );
			}
	...
	private function create_custom_fields( $post_id, $data ) {
		...
		foreach ( $custom_fields as $custom_field ) {
			...
			if ( $fields[0] == 'cost_meta' ) {
				add_post_meta( $post_id, urldecode( 'cost-meta' ), urldecode ( $fields[1] ) );
			} else {
				add_post_meta( $post_id, urldecode( $fields[0] ), urldecode ( $fields[1] ) );
			}
		}
	}
