		...
		<form action="options.php" method="post">
			...
			<p class="submit">
				<input name="Submit" type="submit" class="button-primary"
				       value="<?php esc_attr_e( 'Save Changes', 'skyword-plugin' ); ?>"/>
			</p>
		</form>
	</div>
	<?php
...
function skyword_register_settings() {
    ...
    add_settings_field( 'skyword_coauthors_friendly_pages', ' ', 'skyword_coauthors_friendly_pages_input', __FILE__, 'skyword_generate_new_users_section' );
...
function skyword_coauthors_friendly_pages_input() {
    global $coauthors_plus;
    $options = get_option( 'skyword_plugin_options' );

    if(null !== $coauthors_plus)
	    echo '<input type="checkbox" id="friendly_pages" name="skyword_plugin_options[skyword_coauthors_friendly_pages]" value="1" ' . checked( 1, $options['skyword_coauthors_friendly_pages'], false ) . '/> (BETA) Assign meaningful slugs to Coauthors Plus author pages.<p>New Coauthors Plus pages will receive a slug for their page based on their byline.</p>';
    else
    	echo '<input type="checkbox" id="friendly_pages" name="skyword_plugin_options[skyword_coauthors_friendly_pages]" value="1" ' . checked( 1, $options['skyword_coauthors_friendly_pages'], false ) . ' disabled/><span style="color:lightgrey"> (BETA) Assign meaningful slugs to Coauthors Plus author pages. (Co-Authors Plus plugin disabled, so this option has been disabled)</span><p style="color:lightgrey">New Coauthors Plus pages will receive a slug for their page based on their byline.</p>';
}
...
function skyword_plugin_options_validate( $input ) {
    ...
    $options['skyword_coauthors_friendly_pages'] = empty( $input['skyword_coauthors_friendly_pages'] ) ? false : true;
